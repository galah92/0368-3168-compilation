...
public class FuncDec extends ClassField
{
    ...
    public IRReg toIR()
    {
        ...
        IR.add(new IR.addi(IRReg.sp, IRReg.sp, -(numLocals + 2) * 4));  // allocate stack
        IR.add(new IR.sw(IRReg.s0, IRReg.fp, -1 * 4));  // save s0
        IR.add(new IR.sw(IRReg.s1, IRReg.fp, -2 * 4));  // save s1
        ...
        if (!isMain)
        {
            IR.add(new IR.lw(IRReg.s0, IRReg.fp, -1 * 4));  // retrieve s0
            IR.add(new IR.lw(IRReg.s1, IRReg.fp, -2 * 4));  // retrieve s1
            IR.add(new IR.addi(IRReg.sp, IRReg.sp, (numLocals + 2) * 4));  // deallocate stack
